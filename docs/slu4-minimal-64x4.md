...
| Name | Start | End | Title | Description |
| :-- | :-- | :-- | :-- | :-- |
| `ZERO_PAGE` | 0x0000 | 0x00FF | Zero Page Memory | Fast-access zero page for direct addressing and shared scratch space. |
| `ZERO_PAGE_APPS` | 0x0000 | 0x003F | Zero Page Application Memory | Application-reserved scratch bytes within the zero page. |
| `ZERO_PAGE_LIBS` | 0x0040 | 0x007F | Zero Page Library Memory | Library-reserved zero page range for shared routines. |
| `ZERO_PAGE_OS` | 0x0080 | 0x00FF | Zero Page Operating System Memory | Operating system zero page workspace and vectors. |
| `USER_APPS` | 0x8000 | 0xEFFF | User Application Memory | Main RAM window reserved for user application code and data. |

## Predefined Constants

| Name | Value | Type | Size (Words) | Description |
| :-- | :-- | :-- | :-- | :-- |
| `_Start` | 0xF000 | subroutine |  | Start vector of the OS in RAM. |
| `_Prompt` | 0xF003 | subroutine |  | Hands back control to the input prompt. |
| `_MemMove` | 0xF006 | subroutine |  | Moves memory area (may be overlapping). |
| `_Random` | 0xF009 | subroutine |  | Returns a pseudo-random byte (see _RandomState). |
| `_ScanPS2` | 0xF00C | subroutine |  | Scans the PS/2 register for new input. |
| `_ResetPS2` | 0xF00F | subroutine |  | Resets the state of PS/2 SHIFT, ALTGR, CTRL. |
| `_ReadInput` | 0xF012 | subroutine |  | Reads any input (PS/2 or serial). |
| `_WaitInput` | 0xF015 | subroutine |  | Waits for any input (PS/2 or serial). |
| `_ReadLine` | 0xF018 | subroutine |  | Reads a command line into `_ReadBuffer`. |
| `_SkipSpace` | 0xF01B | subroutine |  | Skips whitespaces (<= 39) in command line. |
| `_ReadHex` | 0xF01E | subroutine |  | Parses command line input for a HEX value. |
| `_SerialWait` | 0xF021 | subroutine |  | Waits for a UART transmission to complete. |
| `_SerialPrint` | 0xF024 | subroutine |  | Transmits a zero-terminated string via UART. |
| `_FindFile` | 0xF027 | subroutine |  | Searches for file <name> given by `_ReadPtr`. |
| `_LoadFile` | 0xF02A | subroutine |  | Loads a file <name> given by `_ReadPtr`. |
| `_SaveFile` | 0xF02D | subroutine |  | Saves data to file <name> defined at `_ReadPtr`. |
| `_ClearVRAM` | 0xF030 | subroutine |  | Clears the video RAM including blanking areas. |
| `_Clear` | 0xF033 | subroutine |  | Clears the visible video RAM (viewport) |
| `_ClearRow` | 0xF036 | subroutine |  | Clears the current row from cursor pos onwards |
...
| Operand | Syntax | Value | Addressing Mode | Description |
| :-- | :-- | :-- | :-- | :-- |
| Zero Page Address (`zp_addr`) | `zp_addr` | numeric expression expressed as 8 bit value | Address | one byte is interpreted as a zero page address to a word that in turn is used as a relative address.<br><br>Valid within `ZERO_PAGE` memory zone.<br><br>Uses only the least significant bits of the address value. |
...
### `PLS4` : Pulls 4 bytes from the stack
...
### `MVS2` : Move word from zero-page to stack at offset
...
### `PHS4A` : Push Long in Memory onto Stack
...
### `PLS2` : Pulls 2 bytes from the stack
