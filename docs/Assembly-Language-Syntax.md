...
| `.16byte` | 16 bytes | Variable | Default |
...
Include paths may include relative directories (e.g. `common/serial.asm`) and the POSIX path segments `.` and `..`. Resolution is performed relative to the file containing the `#include` first; if the file is not found there, the configured include directories are searched.
...
#print <min-verbosity> <color> "message"
#print <color> "message"
```

Behavior:
- Prints the quoted string to standard output.
- If `<color>` is provided, the message is colorized in the terminal output.
...
- If `<min-verbosity>` is provided, the message is printed only when the assembler's log verbosity is greater than or equal to that integer as set by the `-v` command line option. If omitted, the message is always eligible to print (subject to the two controls above).
 - Allowed `<color>` values are: `black`, `red`, `green`, `yellow`, `blue`, `magenta`, `cyan`, `white`.

Examples:
```asm
#print "building utilities"
#print 2 "detailed build step"
#print yellow "warning-style message"
#print 2 cyan "detailed info"
...
#### Compile-time Errors
The `#error` preprocessor directive emits an error message and immediately stops compilation.
...
Behavior:
- Always emits an error message (no verbosity gating).
- The message is prefixed with `ERROR: <line info> -`.
- Respects [conditional compilation](#compilation-control) and [bytecode emission control](#bytecode-emission-control): it only triggers when the current block is active and not muted.
- If no message is provided, a default message of `encountered error directive` is used.
